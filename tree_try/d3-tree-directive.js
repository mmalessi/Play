...
function AngularD3multiParentDirective() {
    ...
    function link(scope, element, attr) {
        ...
        var svg = d3.select(element[0]).append("svg:svg").attr("width", graph.width).attr("height", graph.height).append("svg:g").attr("transform", "translate(-" + graph.width * 0.01 + ", 0)");
        ...
        //ranges to define
        var circleColor = d3.scale.ordinal().domain(["xsmall", "small", "medium", "large", "xlarge", "xxlarge", "none"])
  .range(["#EA5454", "#EAD354", "#EA9854", "#54EA75", "#547EEA", "#A354EA", "black"]);
        
        var circleSize = d3.scale.ordinal().domain(["xsmall", "small", "medium", "large", "xlarge", "xxlarge", "none"])
  .range([5, 7.5, 10, 12.5, 15, 17.5, 2.5])    ;
        ...
        node.append("svg:circle").attr("r", function(d){return circleSize(d.size)}).attr("fill", function (d) { if (d.user_facing == "no") { return "white"} else{return circleColor(d.size)}
            }).attr("stroke", function(d){return circleColor(d.size)}).attr("stroke-width", "2px");
